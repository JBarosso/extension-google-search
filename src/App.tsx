...
function App() {
    ...
    const filteredRows = useMemo(() => {
        if (filters.length === 0) return gsc.rows;

        return gsc.rows.filter(row => {
            return filters.every(filter => {
                const rowValue = filter.field === 'keys' ? row.keys[0] : row[filter.field];
    ...
    return (
        <div className="min-h-screen bg-background-light dark:bg-background-dark text-gray-900 dark:text-gray-100 flex flex-col font-sans transition-colors duration-200">
            ...
            <main className="flex-1 p-4 flex flex-col gap-6 w-full mx-auto">
                {/* Auth Section */}
                <section>
                    <AuthButton
                ...
                {auth.isAuthenticated && (
                    <>
                        ...
                        <div className={activeTab === 'perf' ? 'block' : 'hidden'}>
                            <section className="flex flex-col gap-4 bg-white dark:bg-gray-900 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800">
                                ...
                                <FilterBuilder
                                    ...
                                    disabled={gsc.loading || gsc.rows.length === 0}
                                />
                            ...
                            <section className="flex-1 flex flex-col gap-4 mt-4">
                                ...
                                <DataTable rows={filteredRows} loading={gsc.loading} />
                                ...
                                {filteredRows.length > 0 && (
                                    <div className="sticky bottom-0 z-10 w-full bg-background-light dark:bg-background-dark px-2 py-4 border-t border-gray-100 dark:border-gray-800 shadow-lg">
                                        <ExportActions
                                            data={filteredRows}
                                            filenamePrefix={selectedSite ? new URL(selectedSite).hostname.replace('www.', '') + '_perf' : 'search_console_perf'}
                                            type="performance"
                                            disabled={gsc.loading}
                                        />
                                    </div>
                        ...
                        <div className={activeTab === 'inspect' ? 'block' : 'hidden'}>
